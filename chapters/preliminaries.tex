...
\]
whose elements are unitary matrices with determinant equal to one,
and the projective unitary group
\[
PU(n)=U(n)/Z(U(n)),
\]
where \(Z(U(n))\) denotes the center of \(U(n)\).
...
\[
Z(U(n))=\{A\in U(n): AU=UA,\ \forall\,U\in U(n)\}.
\]
...
For matrix Lie groups, the Lie bracket is given by the matrix
commutator
\[
[X,Y]=XY-YX,
\]
...
matrices,
\[
\mathfrak{su}(n)=\{X\in M_{n}(\mathbb{C}): X^{*}=-X,\ \operatorname{Tr}(X)=0\}.
\]
